*Diredful.el* provides a simple UI to customize dired mode to use different faces and colors. Faces are chosen based on file extension, file name, or a regexp matching the file line.
